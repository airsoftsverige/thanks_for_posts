...
*
* @package acp
* @version $Id: acp_thanks_refresh.php,v 133 2011-12-04 10:02:51 Палыч$
		...
		switch ($stepr)
		{
			...
			//posts
				...
				$sql = 'SELECT DISTINCT post_id
					FROM ' . THANKS_TABLE . ' ORDER BY post_id ASC';
				$result = $db->sql_query($sql);
...
//				natsort ($posts_thanks);
				...
				for ($i=1, $end = 1 + $maxid / 5000; $i < $end; $i++)
				{
					...
					{
						if ($posts_thanks[$j] > $min && $posts_thanks[$j] < $max && !isset($posts_posts[$posts_thanks[$j]]))
						{
			...
			case '4':
				...
				for ($i=1, $end = 1 + $maxuid / 5000; $i < $end; $i++)
				{
					...
					{
						if ($users_thanks[$j] > $min && $users_thanks[$j] < $max && !isset($users_users[$users_thanks[$j]]))
						{
		...
		$sql = 'UPDATE '. THANKS_TABLE .'
			SET forum_id = (SELECT forum_id FROM '. POSTS_TABLE .' WHERE post_id = '. THANKS_TABLE .'.post_id),
			topic_id = (SELECT topic_id FROM '. POSTS_TABLE .' WHERE post_id = '. THANKS_TABLE .'.post_id),
			poster_id = (SELECT poster_id FROM '. POSTS_TABLE .' WHERE post_id = '. THANKS_TABLE .'.post_id)';
		$db->sql_query($sql);
	}
}
?>