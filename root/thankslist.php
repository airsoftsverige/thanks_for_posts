...
*
* @package phpBB3
* @version $Id: thankslist.php,v 133 2011-12-04 10:02:51 Палыч$
...
switch ($mode)
{
		...
		switch ($give)
		{
			...
			$sql = 'SELECT COUNT(user_id) AS total_match_count
				FROM ' . THANKS_TABLE . '  
				WHERE ' . $db->sql_in_set('forum_id', $ex_fid_ary, true) . ' AND user_id = '.$author_id; 
			$where = 'user_id';
			break;
			...
			$sql = 'SELECT COUNT(DISTINCT post_id) as total_match_count
				FROM ' . THANKS_TABLE . '  
				WHERE ' . $db->sql_in_set('forum_id', $ex_fid_ary, true) . ' AND poster_id = '.$author_id; 
			$where = 'poster_id';
			break;		
		}
